...
def Y(excess_return = True):
    ...
    for i in range(5):
        for j in range(5):
            Y["ME" + str(i) + "BP/ME" + str(j)] = []
    for month, monthly_data in data.groupby(data.index):
        sort_ME = monthly_data.sort_values(by = "ME")
        sort_BP_ME = monthly_data.sort_values(by = "BP/ME")
        ...
        for i in range(5):
            for j in range(5):
                ...
                Y["ME" + str(i) + "BP/ME" + str(j)].append(portfolio_ret)
...
# x = months_list
# plt.plot(x, data["Shibor"], label = "Shibor")
# # plt.plot(x, data["Market"], label = "Market")
# plt.plot(x, data["SMB"], label = "SMB")
# plt.plot(x, data["HML"], label = "HML")
# plt.plot(x, data["ROE"], label = "ROE")
# plt.legend()
#%%
def fit(data):
    y = list(data["ME0 BE/ME0"])
    x = data.loc[:, ["Market", "SMB", "HML", "ROE"]]
    x = sm.add_constant(x)
    model = sm.OLS(y, x)
    result = model.fit()
    return result
#%%
# print(fit(data).summary())
#%%
def plot_fit(data):
    fig = plt.figure(figsize = (12, 8))
    fig = sm.graphics.plot_partregress_grid(fit(data), fig = fig)
#%%
def plot_beta(data):
    sns.lmplot(x = "Market", y = "ME0 BE/ME0", data = data)

#%%
%time monthly_return("ME")
#%%
monthly_data = data.groupby(data.index).get_group("2017-11")

#%%
sort = monthly_data.sort_values(by = "ME")
sort["Code"].head()
#%%
stock_list = ["600882.SH", "300272.SZ"]

#%%
monthly_data[monthly_data["Code"].isin(stock_list)].sum(axis = 0)["Return"]
